...
id: ddialog
    ...
    ColumnLayout {
        ...
        D.KeySequenceEdit {
            ...
            background.visible: edit.keys.length === 0 || conflictText.visible
            backgroundColor: {
                if (showAlertColor || conflictText.text.length > 0)
                    return DS.Style.edit.alertBackground
                else
                    return DS.Style.keySequenceEdit.background
            }
            onKeysChanged: {
                if (focus && edit.keys.length === 0) {
                    dccData.updateKey(-1)
                }
            }
        }
        ...
        Connections {
            ...
            function onKeyConflicted(oldAccels, newAccels) {
                ...
                conflictText.text = dccData.conflictText + ", " + qsTr("Click Add to replace shortcut immediately")
            }
