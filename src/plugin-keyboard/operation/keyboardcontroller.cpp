    ...
    : QObject(parent)
{
    ...
    connect(m_shortcutModel, &ShortcutModel::keyEvent, this, [this](bool press, const QString &shortcut){
        ...
        if (!press) {
            if (shortcut.isEmpty()) {
                // if (current)
                //     qDebug() << "should restore to old keys..." << current->name;
                Q_EMIT requestRestore();
                return;
            }

            if (shortcut == "BackSpace" || shortcut == "Delete") {
                // qDebug() << "clear shortcuts.....";
                Q_EMIT requestClear();
            } else {
                if (conflict) {
                    // have conflict
                    QString name = "Add new shortcut";
                    if (current)
                        name = current->name;
                    ...
                    if (current) {
                        ...
                        // qDebug() << "modifyShortcutEdit..." << current->toString();
                    }
        ...
        qDebug() << "keyEvent" << shortcut;
        if (current)
            current->currentKeys = shortcut;
...
QSortFilterProxyModel *KeyboardController::shortcutSearchModel()
{
    ...
    connect(m_shortcutModel, &ShortcutModel::delCustomInfo, sourceModel, [=](ShortcutInfo *info){
        qDebug() << "delCustomInfo" << info->toString();
        sourceModel->reset();
    });
...
void KeyboardController::deleteCustomShortcut(int index)
{
    if (auto shortcut = m_shortcutModel->shortcutAt(index))
