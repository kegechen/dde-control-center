...
int ShortcutModel::indexOf(ShortcutInfo *shortcut)
{
    int index = 0;
    if (m_systemInfos.indexOf(shortcut) < 0)
        index += m_systemInfos.count();

    if (m_windowInfos.indexOf(shortcut) < 0)
        index += m_systemInfos.count();

    if (m_workspaceInfos.indexOf(shortcut) < 0)
        index += m_workspaceInfos.count();

    if (m_assistiveToolsInfos.indexOf(shortcut) < 0)
        index += m_assistiveToolsInfos.count();

    if (int idx = m_customInfos.indexOf(shortcut) < 0)
        index = -1;
    else
        index += idx;
...
QVariant ShortcutListModel::data(const QModelIndex &index, int role) const
{
    ...
    auto displayKeys = ShortcutModel::formatKeys(info->currentKeys.isEmpty() ? info->accels : info->currentKeys);
